import React, { useState } from "react";
import images from "../../assets/images";
import axios from "axios";



const Analyzer = () => {
  const [resumeText, setResumeText] = useState("");
  const [analysisResult, setAnalysisResult] = useState("");
  const [loading, setLoading] = useState(false);
  const [fileUploaded, setFileUploaded] = useState(false);

  const handleFileUpload = async (e) => {
    const file = e.target.files[0];
    if (file && file.type === "application/pdf") {
      const reader = new FileReader();
      reader.onload = async () => {
        const text = reader.result;
        setResumeText(text);
        setFileUploaded(true); // Hide input
        await analyzeResume(text);
      };
      reader.readAsText(file); // PDF-to-text approximation (can be improved)
    }
  };

  const analyzeResume = async (text) => {
    setLoading(true);
    try {
      const response = await axios.post(
        "http://localhost:5000/api/analyze-resume",
        {
          resumeText: text,
        }
      );
      setAnalysisResult(response.data.result);
    } catch (error) {
      setAnalysisResult("Error analyzing resume.");
      console.error(error);
    }
    setLoading(false);
  };

  return (
    <div className="h-screen flex flex-col items-center justify-center">
      <div className="bg-white h-[75%] w-[90%] mx-auto rounded-lg shadow-lg p-5">
        <div className="flex flex-col items-center justify-center h-full">
          <img
            src={images.analyze}
            alt="Analyzer Icon"
            className="mb-5 h-14 w-14"
          />
          <h1 className="text-2xl font-bold text-center mb-5">
            Resume Analyzer
          </h1>

          {!fileUploaded ? (
            <div>
              <p className="text-center mb-5">
                Upload your resume to analyze its content, formatting, and job
                relevance.
              </p>
              <input
                type="file"
                accept=".pdf,.doc,.docx"
                className="border border-gray-300 rounded p-2 w-full mb-5"
                onChange={handleFileUpload}
              />
            </div>
          ) : loading ? (
            <p>Analyzing...</p>
          ) : (
            <pre className="whitespace-pre-wrap text-left bg-gray-100 p-4 rounded w-full max-w-3xl overflow-auto">
              {analysisResult}
            </pre>
          )}
        </div>
      </div>
    </div>
  );
};

export default Analyzer;
